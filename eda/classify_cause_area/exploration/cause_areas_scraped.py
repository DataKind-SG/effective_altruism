...
class CauseAreasScraped:
    ...
    def start(self):
        ...
        all_cause_areas_lemmatized = web_scrape_df[
            self.cause_area_unique_lemmatized_column_name]
        all_cause_areas_lemmatized = list(
            chain.from_iterable(all_cause_areas_lemmatized))
        all_cause_areas_lemmatized = pd.Series(all_cause_areas_lemmatized)

        total_cause_area_count = all_cause_areas.size

        cause_area_counts_df = pd.DataFrame(
            all_cause_areas.value_counts().reset_index())
        cause_area_counts_df.columns = ['cause area', 'count']
        cause_area_counts_df['percentage'] = \
            cause_area_counts_df['count'].apply(
                lambda count: count / total_cause_area_count)

        print("Total organizations with cause areas: " +
              str(all_cause_areas.size))
        print("Total unique cause areas: " +
              str(all_cause_areas.unique().size))
        print('Total unique lemmatized cause areas: '
              + str(all_cause_areas_lemmatized.unique().size))

        cause_areas_merged_by_lemmatization = \
            set.difference(set(all_cause_areas), set(
                all_cause_areas_lemmatized))
    ...
    def add_unique_cause_areas_columns(self, df):
        ...
        df[self.cause_area_unique_column_name] = \
            self.generate_unique_cause_area_set(
                df, CAUSE_AREAS_SCRAPED_COLUMN_NAME)
        df[self.cause_area_unique_lemmatized_column_name] = \
            self.generate_unique_cause_area_lemmatized(
                df, self.cause_area_unique_column_name)
    ...
    @staticmethod
    def generate_unique_cause_area_lemmatized(web_scrape_df,
                                              cause_area_unique_column_name):
        ...
        return web_scrape_df[cause_area_unique_column_name].apply(
            generate_lemmatized_set)
