...
def rm_non_SEA_cty(data):
    # remove organisations outside of SEA
    sea_cty = ['singapore','indonesia','phillipines','thailand','cambodia','myanmar',
               'laos','malaysia','brunei','vietnam','timor leste','papau new guinea',' ']
    data = data.loc[data['country'].isin(sea_cty)]
    return(data)


path =  path.abspath(path.join("../..","data/output/web_scraping/web_scrape_v3.csv"))
export_path =  path.abspath(path.join("../..","data/output/web_scraping/web_scrape_cleaned.csv"))
